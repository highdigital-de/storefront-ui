...
```
or by importing the list of icon colors from `@highdigital/sharedt/variables/colors` and using the desired color variable, e.g
```html
<template>
  <sf-icon :color="iconColor"/>
</template>
<script>
import { iconColors as colors } from "@highdigital/sharedt/variables/colors";
export default {
  data() {
    return {
